...
function TopCreators() {
  return (
    <div className="mx-auto py-20 lg:py-10 max-w-[1280px] px-[115px] text-color-white lg:px-[72px] sm:px-[30px]">
      ...
      <div className="mt-[60px] sm:mt-10 grid grid-cols-4 lg:grid-cols-2 md:grid-cols-1 gap-[30px] md:gap-[20px] sm:gap-5">
        {topCreators.map((value) => (
          <div
            key={value.id}
            className="relative lg:[&:nth-child(n+7)]:hidden sm:lg:[&:nth-child(n+6)]:hidden"
          >
            <span className="absolute text-16px font-space-mono flex items-center justify-center bg-color-black py-1 px-2.5 rounded-full w-[30px] mt-5 ml-5 lg:ml-3 lg:mt-3 text-color-info">
              {value.id}
            </span>
            ...
            <div className="bg-color-grey md:flex rounded-[20px] items-center lg:gap-5 grid grid-cols-1 p-5 lg:flex">
              <img
                src={value.img}
                alt="user img"
                className=" mx-auto lg:w-[60px] lg:h-[60px] lg:m-0 md:w-[80px] md:h-full"
              />

              <div>
                <p className="text-22px font-semibold font-work-sans mt-5 lg:m-0 md:text-[18px] lg:text-left capitalize text-center">
                  {value.name}
                </p>
      ...
      <button className="hidden xl:hidden items-center md:block gap-3 md:mt-10 h-[60px] px-[50px] border-solid border-2 border-color-violet text-16px rounded-[20px] font-semibold font-work-sans">
        <RocketLaunchVioletIcon />
        View Rankings
      </button>
    </div>
